...
import mtpy.imaging.plot_strike as plotstrike#2d PlotStrike
import mtpy.analysis.staticshift as staticshift
import mtpy.imaging.mtplot_tools as mtplt
...
class PlotWidget(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        if self.mt_obj.Z.frequency is not None:
            self.remove_distortion_num_freq_edit.setText(
                "{0:.0f}".format(self.mt_obj.Z.frequency.size)
            )
    ...
    def rotate_estimate_strike(self):
        ...
        #)
        strike_plot = plotstrike.PlotStrike(
            z_object_list=z_list, tipper_object_list=t_list, plot_yn="n"
        )
    ...
    def interp_apply(self):
        ...
        interp_idx = np.where(
            (interp_freq >= self.mt_obj.Z.frequency.min())
            & (interp_freq <= self.mt_obj.Z.frequency.max())
        )
        ...
        if len(interp_idx) != len(new_period):
            ...
            info = [
                ...
                "Data min = {0:<8.3e} s".format(1.0 / self.mt_obj.Z.frequency.max()),
                "Data max = {0:<8.3e} s".format(1.0 / self.mt_obj.Z.frequency.min()),
                "",
                "Given period range:",
    ...
    def plot(self):
        ...
        plot_period = 1.0 / self.mt_obj.Z.frequency
        plot_period_o = 1.0 / self._mt_obj.Z.frequency
        ...
        if event.mouseevent.button == 3:
            ...
            if self._ax_index == 0 or self._ax_index == 1:
                ...
                try:
                    ...
                    f_index = np.where(self.mt_obj.Z.frequency == 1.0 / data_period)
    ...
    def _get_frequency_range(self, period_01, period_02):
        ...
        prange = np.where((self.mt_obj.Z.frequency >= fmin) & (self.mt_obj.Z.frequency <= fmax))
    ...
    def rect_onselect_od(self, eclick, erelease):
        ...
        for ff in f_idx:
            data_period = 1.0 / self.mt_obj.Z.frequency[ff]
